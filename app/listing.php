...
directory structure.
*/
$startdir = 'localhost/xpert_8/app/webroot/files/';
echo $this->webroot;
/*
...
<div id="container">
  ...
  <div id="listingcontainer">
	...
	if($dirok) {
	?>
	<div><a href="<?php echo $startdir.'?dir='.urlencode($dotdotdir);?>" class="<?php echo $class;?>"><?php echo $html->image("/dlf/dirup.png", array('alt' => "Folder")); ?> <strong>..</strong> <em>&nbsp;</em>&nbsp;</a></div>
	<?php
		if($class=='b') $class='w';
		else $class = 'b';
	}
	...
	for($i=0;$i<$arsize;$i++) {
	?>
	<div><a href="<?php echo $_SERVER['PHP_SELF'].'?dir='.urlencode(str_replace($startdir,'',$leadon).$dirs[$i]);?>" class="<?php echo $class;?>"><?php echo $html->image("/dlf/folder.png", array('alt' => $dirs[$i]));?><strong><?php echo $dirs[$i];?></strong> <em>-</em> <?php echo date ("M d Y h:i:s A", filemtime($includeurl.$leadon.$dirs[$i]));?></a></div>
	<?php
		if($class=='b') $class='w';
		else $class = 'b';	
	}
	...
	?>
	<div><a href="<?php echo $fileurl;?>" class="<?php echo $class;?>"<?php echo $thumb2;?>><?php echo $html->image("/dlf/$icon", array('alt' => $files[$i])); ?><strong><?php echo $filename;?></strong> <em><?php echo round(filesize($includeurl.$leadon.$files[$i])/1024);?>KB</em> <?php echo date ("M d Y h:i:s A", filemtime($includeurl.$leadon.$files[$i]));?><?php echo $thumb;?></a></div>
	<?php
		if($class=='b') $class='w';
		else $class = 'b';	
	}	
